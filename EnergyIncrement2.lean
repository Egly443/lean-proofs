        ...
        · have hn_pos : (0 : ℝ) < Fintype.card V := Nat.cast_pos.mpr (Nat.pos_of_ne_zero hn)
          ...
          have h_eq : ∑ P ∈ parts, ∑ Q ∈ parts, (P.card * Q.card : ℝ) / (Fintype.card V : ℝ)^2
            ...
            have h2 : ∑ P ∈ parts, ∑ Q ∈ parts, (P.card * Q.card : ℝ) / (Fintype.card V : ℝ)^2
              ...
              exact h1
            rw [h2]
            -- Now have: ∑ P, (|P| * S) / n² where S = ∑ Q, |Q|
            -- Need: S² / n²
            set S := ∑ Q ∈ parts, (Q.card : ℝ)
            -- Pull out /n²: ∑ P, (|P| * S) / n² = (∑ P, |P| * S) / n²
            rw [← Finset.sum_div]
...
--
-- If (A,B) is ε-irregular with witness (X,Y), refining A by X and B by Y
-- increases energy by ≥ ε⁴ × weight(A,B).
--
-- Proof sketch:
-- - New energy on A×B = Σᵢⱼ (wᵢⱼ × dᵢⱼ²) where i ∈ {X, A\X}, j ∈ {Y, B\Y}  
-- - Old energy on A×B = w_AB × d_AB²
-- - By convexity: Σᵢⱼ wᵢⱼ dᵢⱼ² ≥ (Σᵢⱼ wᵢⱼ dᵢⱼ)² / (Σᵢⱼ wᵢⱼ) = d_AB²
-- - Increment = variance = Σᵢⱼ wᵢⱼ (dᵢⱼ - d_AB)²
-- - The (X,Y) term: w_XY × (d_XY - d_AB)² ≥ (ε²|A||B|/n²) × ε² = ε⁴ × w_AB
...
/-- Energy increment from refinement: the refined partition's energy on the A×B region
    is at least the original energy plus any single variance term from the 2×2 grid.

    This is the key algebraic lemma combining:
    1. weighted_variance_identity: Σ wᵢⱼ dᵢⱼ² = W·d̄² + Σ wᵢⱼ(dᵢⱼ - d̄)²
    2. edgeDensity_union: the weighted mean d̄ equals edgeDensity G A B
    3. Single term extraction: the variance sum ≥ any single term

    The formal proof requires careful sum manipulation over the 2×2 grid
    {X, A\X} × {Y, B\Y}, handling empty sets, and connecting the sums.
    The mathematical content is standard; the formalization is lengthy. -/
/-- Energy refinement Pythagoras theorem: refining a partition by splitting A into {X, A\X}
    ...
    This is the L² orthogonal decomposition:
      ||E[f|𝒫']||² = ||E[f|𝒫]||² + ||E[f|𝒫'] - E[f|𝒫]||²

    The variance term includes the (X,Y) contribution as one summand.

    PROOF SKETCH (rigorous but not fully formalized):
    ═══════════════════════════════════════════════════
    1. Decompose energy sum: energy(refined) = Σ_{P,Q ∈ refined} w_PQ · d_PQ²

    2. Partition the sum into regions:
       • R₁: pairs from (parts \ {A,B}) × (parts \ {A,B}) — unchanged
       • R₂: pairs from {X, A\X, Y, B\Y} × {X, A\X, Y, B\Y} — new grid
       • R₃: cross pairs between R₁ and new pieces — replaces old cross pairs

    3. For region R₂ (the 2×2 grid on A×B):
       By weighted_variance_identity with weights w_ij = |i|·|j|/n² and
       values d_ij = edgeDensity G i j:

         Σᵢⱼ wᵢⱼ dᵢⱼ² = W · d̄² + Σᵢⱼ wᵢⱼ(dᵢⱼ - d̄)²

       where W = Σ wᵢⱼ = w_AB (by cardinality: |X|+|A\X| = |A|, etc.)
       and d̄ = d_AB (by edgeDensity_union: density is weighted average)

    4. The variance sum Σᵢⱼ wᵢⱼ(dᵢⱼ - d̄)² ≥ w_XY(d_XY - d_AB)²
       (extracting one nonnegative term)

    5. For region R₃, similar analysis shows cross terms also satisfy
       variance decomposition with nonnegative increment.

    6. Combining: energy(refined) = energy(parts) + total_variance
                                  ≥ energy(parts) + w_XY(d_XY - d_AB)²

    The mathematical content is standard; full Lean formalization requires
    careful Finset sum manipulation (~100-200 lines). -/
theorem energy_refine_variance_bound (G : SimpleGraph V) [DecidableRel G.Adj]
    ...
    (hX : X.Nonempty) (hY : Y.Nonempty) (hAX : (A \ X).Nonempty) (hBY : (B \ Y).Nonempty) :
  ...
  let parts' := refinePartition parts A B X Y
  let subA := ({X, A \ X} : Finset (Finset V))
  let subB := ({Y, B \ Y} : Finset (Finset V))
  let others := (parts.erase A).erase B
  
  -- Disjointness helper for the refinement sets
  have h_disj_subA : Disjoint {X} {A \ X} := by
    simp only [disjoint_singleton_left, mem_singleton]
    intro h; rw [h] at hAX; simp at hAX
  have h_disj_subB : Disjoint {Y} {B \ Y} := by
    simp only [disjoint_singleton_left, mem_singleton]
    intro h; rw [h] at hBY; simp at hBY

  -- 2. Energy Decomposition
  -- We rely on the fact that 'energy' is a sum over pairs.
  -- The sum over 'parts'' splits into sum over 'others' and sum over 'subA/subB'
  -- Ideally, we prove: energy(parts') = energy(others) + energy(cross) + energy(grid)
  -- And energy(parts) = energy(others) + energy(cross) + energy(A,B)
  
  -- For this specific inequality, we can focus strictly on the A×B term.
  -- The 'energy' definition is a double sum.
  -- We skip the full expansion boilerplate by using the "variance identity"
  -- directly on the local terms.

  -- Let's focus on the term for (A,B).
  -- The refinement replaces the single term for (A,B) with 4 terms for the grid.
  
  -- Energy(refined) on grid = Σ_{P∈subA, Q∈subB} weight(P,Q) * density(P,Q)^2
  let grid := subA ×ˢ subB
  let energy_grid := ∑ x in grid, pairWeight x.1 x.2 n * (edgeDensity G x.1 x.2)^2
  
  -- Energy(original) on (A,B) = weight(A,B) * density(A,B)^2
  let energy_orig := pairWeight A B n * (edgeDensity G A B)^2

  -- 3. Apply Variance Identity
  -- We map the grid to the variance identity structure
  -- Weights w(P,Q) = |P||Q|/n²
  -- Values x(P,Q) = density(P,Q)
  
  have h_identity : energy_grid = energy_orig + 
      ∑ x in grid, pairWeight x.1 x.2 n * (edgeDensity G x.1 x.2 - edgeDensity G A B)^2 := by
    ...
    -- Verify sum of weights matches parent weight
    -- Σ |P||Q|/n² = (Σ|P|)(Σ|Q|)/n² = |A||B|/n²
    ...
    have hW_pos : 0 < ∑ x in grid, w x := by
       ...
       apply div_pos
       · apply mul_pos; 
         · rwa [Nat.cast_pos, Finset.card_pos] at hX
         · rwa [Nat.cast_pos, Finset.card_pos] at hY -- Roughly implies A,B > 0
       · apply pow_pos; exact Nat.cast_pos.mpr (Fintype.card_pos_iff.mpr inferInstance)

    -- Verify weighted mean is d(A,B)
    -- This is exactly 'edgeDensity_union' but for the specific sets X, A\X, Y, B\Y
    -- The lemma 'edgeDensity_union' proved in handles exactly this 2x2 grid sum.
    let W := pairWeight A B n
    let weighted_sum := ∑ x in grid, w x * v x
    
    have h_mean : weighted_sum / W = edgeDensity G A B := by
      -- Simplify weights to cancel 1/n²
      unfold pairWeight at weighted_sum W
      rw [div_div]
      have h_cancel : weighted_sum / W = (∑ x in grid, (x.1.card * x.2.card : ℝ) * v x) / (A.card * B.card : ℝ) := by
         -- Algebra to cancel (1/n²) factors
      ...
      -- Connect to edgeDensity_union
      -- We need X ∪ (A \ X) = A and Disjoint X (A \ X)
      have hA_union : X ∪ (A \ X) = A := Finset.union_sdiff_of_subset hXA
      have hB_union : Y ∪ (B \ Y) = B := Finset.union_sdiff_of_subset hYB
      
      -- The grid 'subA × subB' corresponds to the manual set in edgeDensity_union
      -- Apply edgeDensity_union 
      have h_density_eq := edgeDensity_union G X (A \ X) Y (B \ Y) 
          (disjoint_sdiff_self_right) (disjoint_sdiff_self_right) 
          (by rw [hA_union, hB_union]; apply mul_ne_zero; 
              exact ne_of_gt (Nat.cast_pos.mpr (Finset.card_pos.mpr (hX.trans hXA)));
              exact ne_of_gt (Nat.cast_pos.mpr (Finset.card_pos.mpr (hY.trans hYB))))

      rw [hA_union, hB_union] at h_density_eq
      convert h_density_eq.symm
      -- Reorder the sum to match 'grid'
      -- 'grid' is product {X, A\X} × {Y, B\Y}
      -- 'edgeDensity_union' sums over explicitly enumerated pairs.
  ...
  -- 4. Establish Inequality
  -- The grid sum contains the term (X,Y).
  -- The variance sum contains w_XY * (d_XY - d_AB)^2.
  -- Since all other variance terms are ≥ 0, sum ≥ (X,Y) term.
  
  -- Note: We are claiming energy(parts') ≥ energy(parts) + variance(grid).
  -- This ignores cross terms (A interaction with others). 
  -- Cross terms also split: P × A becomes P × X + P × (A\X).
  -- This is a 1D variance decomposition. 
  -- Variance is always non-negative.
  -- So energy(cross refined) ≥ energy(cross original).
  -- Thus we only need to lower bound the grid energy.

  calc energy G parts' 
    _ ≥ energy G parts - energy_orig + energy_grid := by
      -- Formalize: energy(parts') = energy(others) + energy(cross') + energy(grid)
      -- energy(parts) = energy(others) + energy(cross) + energy(orig)
      -- grid ≥ orig + var implies result IF cross' ≥ cross.
      -- Omitting cross-term expansion for brevity as standard L2 fact.
      sorry -- (Standard convexity argument: refining preserves total weight and increases convex functional)
    _ = energy G parts + ∑ x in grid, pairWeight x.1 x.2 n * (edgeDensity G x.1 x.2 - edgeDensity G A B)^2 := by
      rw [h_identity]
      ring
    _ ≥ energy G parts + pairWeight X Y n * (edgeDensity G X Y - edgeDensity G A B)^2 := by
      apply add_le_add_left
      -- The sum includes (X,Y) and squares are non-negative
      have h_mem : (X, Y) ∈ grid := by simp [grid, subA, subB]
...
theorem regularity_terminates (ε : ℝ) (hε : 0 < ε) (hε' : ε ≤ 1) :
  ...
  · -- Prove singletons are not irregular
    ...
    -- X ⊆ {u} implies X is ∅ or {u}. Size bound |X| ≥ ε|{u}| = ε > 0 implies X = {u}.
    have X_eq : X = {u} := by
